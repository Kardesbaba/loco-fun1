...
public class DynamicsDatabase {
    ...
    public boolean initializeConnection() {
        try {
            ...
            config.setJdbcUrl("jdbc:mysql://" + Main.hostDB + ":" + Main.portDB + "/" + Main.nameDB + "?characterEncoding=UTF-8&useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC");
            config.setDriverClassName("com.mysql.cj.jdbc.Driver"); // Driver actualizado
            config.setUsername(Main.userDB);
            config.setPassword(Main.passDB);
            config.setAutoCommit(true); // AutoCommit activado
            config.setMaximumPoolSize(10); // Igual que tu configuración vieja
